dimension = [int(i) for i in str(input("Input grid size: ")).split(" ")]
...
while True:
    print("-----------------------")  
    target = [int(i) for i in str(input("Input target: ")).split(" ")]
    
    if target == [-1]: break
    
    elif (target[0] >= dimension[0] or target[0] <= 0) or (target[1] >= dimension[1] or target[1] <= 0): 
        print("TARGET ERROR! Please input again.")
        continue
    
    while True:
        diamond_type = str(input("Type of target: ")).lower() 
        if diamond_type == "b" or diamond_type == "l":
            if diamond_type == "b": flaming(target[0], target[1])
            if diamond_type == "l": lightning(target[0], target[1])
            break
        else:
            print("TYPE ERROR! Please input again.")
            continue
...
for i in displayboard: 
    ...
    if int(displayboard[-1][-1]) >= 10: BigBoard = True
    lines = ''
    for char in i:
        lines += char
        if BigBoard: lines += "  "
